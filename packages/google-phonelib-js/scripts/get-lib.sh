...
folder_to_copy="javascript/i18n/phonenumbers"
destination_dir="./glib"

# Check if destination directory exists and remove it
if [ -d "$destination_dir" ]; then
  echo "Destination directory '$destination_dir' exists. Removing..."
...
# Copy the *contents* of the specified folder to the destination directory
if [ -d "$extracted_folder/$folder_to_copy" ]; then
  cp -r "$extracted_folder/$folder_to_copy/"* "$destination_dir/"

  if [ $? -ne 0 ]; then
    echo "Error copying contents of folder '$folder_to_copy' to '$destination_dir'."
...
echo "Contents of folder '$folder_to_copy' copied to '$destination_dir' successfully."